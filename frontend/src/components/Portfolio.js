    ...
    const fetchPortfolio = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/trade/portfolio`,
...
const fetchTransactions = async (symbol) => {
    try {
      const res = await axios.get(
        `http://localhost:5000/trade/portfolio/${symbol}/transactions`,
        { headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` } }
      );
              ...
              {portfolio.map((item,index)=>{
                ...
                return (
                  <React.Fragment key={index}>
                    ...
                    <td
                    className={`pl-cell ${isProfit ? "profit" : "loss"}`}>
                      {isProfit ?(
                        <>
                          <ArrowUpRight className="arrow-icon"/>₹
                          {item.profitLoss.toFixed(2)} ({item.profitLossPercent}%)
                        </>
                      ) :(
                        <>
                          ...
                          <ul className="transaction-list">
                            {transactions[item.symbol].map((t,i)=>(
                              <li key={i}>
                                ...
                                <b>{t.quantity} </b>Buy at <b> ₹{t.price.toFixed(2)} </b>  on{"   "}
                               <b> {new Date(t.date).toLocaleDateString()}</b>
                              </li>
                            ))}
                          </ul>
                        ):(
